...
interface UniformDef {
  ...
  extraSwatches?: string[]; // added for Riso's traditional-CMY option (§4.2)
}
```
...
**Sectioned scale — QA pass, applies to Pixelated/Dither/Halftone/ASCII.** Walker asked for a toggle that slices the canvas into sections, using a different scale of the effect per section. Added a `visibleWhen` field to `UniformDef` (§4.1's contract) so the single `cellsAcross` slider and the sectioned controls don't both show at once — only one set is visible depending on the `sectioned` toggle. Off by default — no change to prior behavior for any of the four shaders.

Originally a fixed 2x2 grid (4 quadrants), one independent scale slider per quadrant. Revised per Walker's follow-up QA to a user-controlled grid: `sectionRows` x `sectionCols` sliders (1-10 each, independent), a `sectionDirection` enum (`left-right` / `top-bottom` / `radial`), and two value sliders — `sectionLow`/`sectionHigh` — interpolated across the grid per that direction rather than one slider per section. `left-right` and `top-bottom` are linear gradients by column/row index; `radial` is low-at-center, high-at-edge by distance from the grid's center. Shared implementation: `shaders/sectionedScale.ts` (the uniformSchema entries) and a `sectionedScale()` GLSL helper in the shader preamble (`gl/renderer.ts`) that computes each fragment's section, then interpolates.
...
- **ASCII** — samples cell luminance, indexes into a glyph atlas. Params: cells across, glyph set, fg/bg, gamma, invert, **contrast** (added after #17's initial build — Walker's feedback: the grid read too loose and the tonal range too flat; contrast stretches the luminance curve so the full glyph ramp, space to `@`, actually gets used). **Decided:** no T/O/M-specific character set — glyph set is standard ASCII characters, built fresh rather than ported from prior work. Went through three revisions: a 70-character luminance-ordered ramp (Paul Bourke's grayscale set) first, for finer tonal resolution than the original 10-char ramp; then, per Walker's follow-up QA (wanted a smaller set but more contrast between darkest and lightest points), back to the 10-char ramp `' .:-=+*#%@'` extended past `@` with the four Unicode block-shade glyphs `░▒▓█`; then a third pass dropping the fully solid block (`█`) after review — it read as a flat color fill rather than a character — leaving `' .:-=+*#%@░▒▓'` as the current set, three block-shade glyphs (light to dark) carrying the dense end instead of four.
...
*4th revision (QA pass) — back to atlas-based, but not #17's atlas.* Walker supplied 5 custom SVG assets (`src/shaders/patterns/1-lightest.svg` … `5-dark.svg`, numbered light-to-dark) to fully replace the 5 procedural bands; band 6 becomes a flat solid fill for the darkest tonal range, dropping the herringbone/7th tier (6 bands total, down from 7). Unlike the 1st revision's dropped atlas approach, the two-coordinate-grid system from the 2nd revision (strict `cellsAcross` grid for band assignment, separate finer grid for pattern tiling) already decouples pattern frequency from luminance-sampling resolution, so sampling an atlas texture no longer has the problem that killed the original atlas plan — it just needed the tiling math the procedural functions already had, pointed at a texture instead of a formula.

The 5 SVGs are opaque, two-tone tiles with their own baked-in brand-palette colors (not transparent coverage masks) — confirmed with Walker: render them exactly as designed rather than tinting through a per-band bg/fg picker like before, so the color pickers for bands 1-5 are gone; only band 6 (solid, no asset) keeps one. `patternAtlas.ts` rasterizes the SVGs into a texture atlas synchronously via Canvas 2D + a minimal `<rect>`/`<circle>` parser (DOMParser, no new dependency) — deliberately not a general SVG renderer, only what the current assets need. New pattern assets: same two folders/naming convention (`src/shaders/patterns/`, numbered light-to-dark, one motif per file, `<rect>`/`<circle>` only), swap the file and re-point the import in `patternFill.ts`.
...
**Ink colors — brand palette by default, traditional CMY also offered.** Walker asked specifically for traditional process cyan/magenta/yellow to remain available on Riso's three ink params, alongside the brand-palette defaults (§4.1/§9) — real full-color reconstruction in `channels` mode needs true C/M (each RGB channel's clean inverse), which the brand palette doesn't contain. Added `extraSwatches` to the `UniformDef` contract for this: a param can offer additional swatches beyond the global brand palette without reopening every color param in the app to arbitrary input. Scoped to Riso's ink params only.
...
```json
{
  ...
  "vector": {
    "id": "scribbles",
    "params": { "asset1": "scribble-01", "x1": 0.3, "y1": 0.5, "rotation1": 0, "scale1": 1,
                "enableSecond": true, "asset2": "scribble-02", "x2": 0.7, "y2": 0.5, "rotation2": 90, "scale2": 1.4,
